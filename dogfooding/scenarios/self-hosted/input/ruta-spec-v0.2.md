...
- **R-11.4.1** Before each release, ruta MUST be used (by the maintainer, on a real spec) to complete at least the scope of that release. The resulting artifacts MUST be committed to the repository under `dogfooding/v<release>/` so that the claim is externally auditable. A release whose `dogfooding/v<release>/` directory is missing or empty MUST NOT be published.
- **R-11.4.2** ruta's own spec (this document, in later versions) is a valid dogfooding target.
...
- A user can run `pi install npm:@<ns>/pi-ruta`, then `/ruta-init spec/example.md`, then complete Days 0–3 of the protocol, producing non-empty `notebook.md`, `glossary.md`, and `gaps.md` files.
...
- Made R-11.4.1 dogfooding verifiable (artifacts committed to `dogfooding/v<release>/`).
